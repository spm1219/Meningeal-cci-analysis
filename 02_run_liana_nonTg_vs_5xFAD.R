...
counts_file      <- "counts_NonTgvs5xFAD.csv"
meta_file        <- "meta_NonTgvs5xFAD.csv"
cluster_map_file <- "AD_cluster_map.csv"
output_nontg     <- "liana_NonTg.csv"
output_5xfad     <- "liana_5xFAD.csv"
...
run_liana_and_save <- function(seu_obj, keep_source, output_file) {
  sub_obj <- subset(seu_obj, subset = source == keep_source)
  if (ncol(sub_obj) == 0) {
    stop("No cells found for source == '", keep_source, "'.")
  }

  Idents(sub_obj) <- sub_obj@meta.data$cell_type
...
run_liana_and_save(seu, keep_source = "NonTg", output_file = output_nontg)
run_liana_and_save(seu, keep_source = "5xFAD", output_file = output_5xfad)
